...
from keyboards.admin_inline_keyb import categories_kb_4_products
from keyboards.client_keyboards import next_prev_kb, get_next_prev_keyboard
from state.admin_states import ProductStates
from state.states import ShowStates
from utils.database import Database
from utils.lotin_l import lotin_l, to_lower
...
@product_router.message(Command('search'))
async def all_products_handler(message: Message, state: FSMContext):
    # categories = db.get_categories()

    await state.set_state(ShowStates.searchstate)
    await message.answer("Please select a category to view the products.")


@product_router.message(ShowStates.searchstate)
async def show_search_p(message: Message, state: FSMContext):
    products = db.get_title_desc(message.text)
    await state.set_state(ShowStates.showsearchProductsState)
    await state.update_data(products=products)
...
@product_router.callback_query(ShowStates.showsearchProductsState)
async def show_p_s_callback_query(query: CallbackQuery, state: FSMContext):
    index = int(query.data)
    all_data = await state.get_data()
    products = all_data['products']
