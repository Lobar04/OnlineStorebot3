...
class Database:
    ...
    def get_title_desc(self,a):
        product = self.cursor.execute(
            "SELECT id, product_title, product_text, product_image, product_price, product_phone FROM products WHERE product_title LIKE ? OR product_text LIKE ?;",
            (f"%{a}%", f"%{a}%")
        )
        return product.fetchone()